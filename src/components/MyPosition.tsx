        ...
        <ul className="mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {FIGURES.map(f => (
            <li key={f.id}>
              <a
                href={`#/position/${f.id}`}
                className="flex items-center gap-3 rounded-2xl bg-white/5 border border-white/10 p-3 sm:p-4 hover:bg-white/10 hover:border-vsg-400/60 transition"
              >
                <span
    ...
    <section className="max-w-[1400px] mx-auto px-5 sm:px-8 py-6 sm:py-10">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="eyebrow text-white/50">Dein Läufersystem als</p>
